...
class Player:
    ...
    def walk(self) -> tuple[str, ...]:
        ...
        pos = self.player_positions.get(self.player_index, (0, 0))
        prompt = (
            f"你是角色 {self.role_type}（ID {self.player_index}），"
            f"当前坐标 {pos}，地图：{self.map}，历史记录：{history}。\n"
            "请生成 0–3 步的移动，格式 Up,Down,Left,Right，用逗号分隔，仅返回指令。"
        )
        resp = askLLM(prompt)
        dirs = [d.strip() for d in resp.split(",") if d.strip() in {"Up", "Down", "Left", "Right"}]
        return tuple(dirs[:3])
